...
<%
    ...
    try
    {
        ...
        {  
            ...
            if( row.getCell(4)==null||row.getCell(4).toString().isEmpty()==true)                       ico3 = 0;
            else                                            ico3 = Float.parseFloat(row.getCell(4).toString());

            if( row.getCell(6)==null||row.getCell(6).toString().isEmpty()==true)                       ico4 = 0;
            else                                            ico4 = Float.parseFloat(row.getCell(6).toString());

            if( row.getCell(9)==null||row.getCell(9).toString().isEmpty()==true)                       aco3 = 0;
            else                                            aco3 = Float.parseFloat(row.getCell(9).toString());

            if( row.getCell(10)==null||row.getCell(10).toString().isEmpty()==true)                       aco4 = 0;
            else                                            aco4 = Float.parseFloat(row.getCell(10).toString());
            ...
            if(regno_name.containsKey(Long.parseLong(rollno)))
            {
                String sql=null;
                if(ico3!=-1&&ico4!=-1){
                sql="insert into iat2(regno,name,co3,co4) values('"+regno+"','"+name+"','"+ico3+"','"+ico4+"')";
                st.executeUpdate(sql);}
               
                if(aco3!=-1&&aco4!=-1){
                sql="insert into ass2(regno,name,co3,co4) values('"+regno+"','"+name+"','"+aco3+"','"+aco4+"')";
                st.executeUpdate(sql);}

            }
        }
        response.sendRedirect("iat2.jsp");
    }
    catch(Exception e)
    {
        out.println(e);
